...
public class KeySettingController : MonoBehaviour
{
    ...
    {
        mOriginKeyCode = Managers.KeyInput.GetKeyCode(mKeyBindingName);
        mKeyButtonText.text = (mOriginKeyCode).ToString().ToUpper();
    }
            ...
            if (Input.anyKeyDown)
            {
                foreach (KeyCode kcode in Enum.GetValues(typeof(KeyCode)))
                {
                    if (Input.GetKey(kcode))
                    {
                        ...
                        // 키 설정을 할 수 있는경우?
                        if (Managers.KeyInput.CheckKey(kcode, mOriginKeyCode))
                        {
                            // 키 지정
                            Managers.KeyInput.AssignKey(kcode, mKeyBindingName);
                            mOriginKeyCode = kcode;

                            // 키 레이블을 변경
                            mKeyButtonText.text = (kcode).ToString().ToUpper();
